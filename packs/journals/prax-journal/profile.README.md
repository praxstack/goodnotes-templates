...
```
packs/journals/prax-journal/profile.json
packs/journals/prax-journal/profile.*.json      # profile.local.json, profile.bak.json …
!packs/journals/prax-journal/profile.example.json  # explicit allow for the committable seed
```
...
fails CI if the committed example ever contains real names (`<REDACTED-PSYCH-FIRST>`,
`<REDACTED-PSYCH-FIRST-DR>`, `Prax`). Belt-and-braces: the gitignore prevents commit, the
test prevents accidental copy-paste.

## Bootstrap (one-time setup)

```bash
# From the repo root, inside packs/journals/prax-journal/
...
npx tsx -e "
  import('./src/types/profile.js').then(async ({ parseProfile }) => {
    ...
    const p = parseProfile(JSON.parse(readFileSync('packs/journals/prax-journal/profile.json', 'utf8')));
