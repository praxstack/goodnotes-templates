/**
 * Generate full year ADHD v3 planner.
 * Usage: npx tsx scripts/generate-v3-year.ts [colorMode] [year]
 * Example: npx tsx scripts/generate-v3-year.ts        (default light, 2026)
 * Example: npx tsx scripts/generate-v3-year.ts dark    (dark mode, 2026)
 */
...
async function main() {
  console.log(`\n📅 Generating ADHD v3 Full Year Planner — ${year}${colorMode ? ` (${colorMode})` : ''}\n`);
  ...
  const result = await generateDailyYearPlannerV2({
    ...
    locale: 'en',
    colorMode,
    outputPath: `output/templates/adhd-v3-planner-${year}${colorMode ? `-${colorMode}` : ''}.pdf`,
  });
