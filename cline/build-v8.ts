/**
 * build-v8.ts — Prax Journal v8 · Beautified 30-Day Bundle (Cline)
 *
 * v8 = v7 content, made warmer and scaled to a month. Three things, additive only
 ...
 *     (truth/quote/pill), then a v8-master-bundle.pdf concatenating the beautified
 ...
 *
 * Run: ./node_modules/.bin/tsx cline/build-v8.ts
 * Open: open cline/output/v8/v8-master-bundle.pdf
 */
...
const V8 = path.join(OUT, 'v8');
const DAILY_DIR = path.join(V8, 'daily');
const SHEETS_DIR = path.join(V8, 'sticker-sheets');
...
// ── Shared stylesheet — locked v6/v7 design language + v8 beautification ─────
const CSS = `
:root{
...
/* v8 beautification — faint, page-content-aware watermark + corner sprig.
...
// ── v8 beautification: faint page-content-aware watermarks ──────────────────
...
async function main() {
  ...
  const report: QA[] = [];
  // cache rendered single-page PDFs keyed by `${kind}__m${motif}`
  const cache = new Map<string, Uint8Array>();
  const dailyBundle = await PDFDocument.create();

  try {
    // 1) render the 4 page-types × 6 motifs = 24 base pages (QA gated)
    for (const kind of PAGE_TYPES) {
      for (let motif = 0; motif < MOTIFS; motif++) {
        const key = `${kind}__m${motif}`;
        const { pdf, qa } = await renderAndVerify(browser, key, bodyFor(kind, motif, 0));
        report.push(qa);
        cache.set(key, pdf);
      }
    }

    // 2) compose 30 day PDFs (4pp each) + the 120pp daily bundle from cache
    for (let day = 1; day <= DAYS; day++) {
      ...
      for (const kind of PAGE_TYPES) {
        ...
        const [bp] = await dailyBundle.copyPages(src, [0]); dailyBundle.addPage(bp);
      }
      dayDoc.setTitle(`Prax Journal v8 — Day ${String(day).padStart(2, '0')}`);
      await fs.writeFile(path.join(DAILY_DIR, `day-${String(day).padStart(2, '0')}.pdf`), await dayDoc.save({ useObjectStreams: false }));
    }
    dailyBundle.setTitle('Prax Journal v8 — 30-Day Daily Bundle (beautified)');
    const dailyBundleBytes = await dailyBundle.save({ useObjectStreams: false });
    await fs.writeFile(path.join(V8, 'daily-30-day-bundle.pdf'), dailyBundleBytes);
  } finally {
    await browser.close();
  }

  // QA gate (only on the 24 freshly-rendered base pages)
  console.log('\n[v8] daily render-QA (4 page-types × 6 motifs):');
  let allPass = true;
  for (const q of report) { const ok = q.verdict === 'pass'; allPass = allPass && ok; if (!ok) console.log(`  ✗ ${q.key.padEnd(16)} overflow=${q.overflowPx}px footer=${q.footerCollision} minFont=${q.minFontPx}px → FAIL`); }
  console.log(`  ${allPass ? '✓ all 24 base pages pass' : '✗ some pages failed'} (A4 / overflow / footer / legibility)`);
  if (!allPass) { console.error('[v8] FAIL — fix daily overflow/collision/font before shipping.'); process.exit(2); }

  // 3) printable sticker sheets from the existing 65 PNGs
  ...
  console.log(`[v8] sticker sheets: ${sheetParts.map(s => s.name).join(', ') || 'none found'}`);

  // 4) v8 master bundle = daily-30-day + sticker sheets + existing v7 tools/pomodoro/extras
  const master = await PDFDocument.create();
  let total = 0;
  total += await mergeInto(master, await fs.readFile(path.join(V8, 'daily-30-day-bundle.pdf')));
  ...
  master.setTitle('Prax Journal v8 — Master Bundle');
  await fs.writeFile(path.join(V8, 'v8-master-bundle.pdf'), await master.save({ useObjectStreams: false }));

  console.log(`\n[v8] master bundle: ${total} pages → cline/output/v8/v8-master-bundle.pdf`);
  console.log(`[v8]   daily: 30 day PDFs + daily-30-day-bundle.pdf (120pp, beautified)`);
  console.log(`[v8]   carried over (v7, locked design): ${merged.join(', ')}`);
  if (missing.length) console.log(`[v8]   not found (run their generators first): ${missing.join(', ')}`);
  console.log('[v8] done — all freshly-rendered daily pages pass A4 / overflow / footer / legibility.');
}

main().catch(err => { console.error('[v8] failed:', err instanceof Error ? err.stack : err); process.exit(1); });
