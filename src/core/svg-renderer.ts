/**
 * SVG sticker generator using @svgdotjs/svg.js + svgdom for server-side rendering.
 *
 ...
 *
 * Sticker types generated:
 * - Date/month tabs, day tabs, number circles
 * - Priority markers, checkboxes, progress bars
 * - Sticky notes, washi tape, banners, dividers, frames
 * - Weather icons, star ratings, arrows
 */
...
// ─── SVG Generation (raw markup, no svgdom dependency) ──────────

/**
 * Generate SVG markup for a sticker. Returns raw SVG string.
 * We use raw SVG markup instead of svg.js to avoid svgdom install issues.
 */
export function generateStickerSVG(
  type: StickerType,
  theme: Theme,
  options: { text?: string; variant?: number } = {}
): string {
  const size = STICKER_SIZES[type];
  const { text, variant = 0 } = options;

  switch (type) {
    case 'date-tab': return generateDateTab(size.width, size.height, theme, text || 'Jan');
    case 'month-tab': return generateMonthTab(size.width, size.height, theme, text || 'January');
    case 'day-tab': return generateDayTab(size.width, size.height, theme, text || 'Mon');
    case 'number-circle': return generateNumberCircle(size.width, size.height, theme, text || '1');
    case 'priority-marker': return generatePriorityMarker(size.width, size.height, theme, text || 'P1');
    case 'checkbox': return generateCheckbox(size.width, size.height, theme, variant);
    case 'progress-bar': return generateProgressBar(size.width, size.height, theme, variant);
    case 'sticky-note': return generateStickyNote(size.width, size.height, theme, variant);
    case 'washi-tape': return generateWashiTape(size.width, size.height, theme, variant);
    case 'banner': return generateBanner(size.width, size.height, theme, text || 'Notes');
    case 'divider': return generateDivider(size.width, size.height, theme, variant);
    case 'frame': return generateFrame(size.width, size.height, theme, variant);
...
function generateDateTab(w: number, h: number, theme: Theme, text: string): string {
  const r = theme.borderRadius;
  return svgWrap(w, h, `
    <rect x="2" y="2" width="${w - 4}" height="${h - 4}" rx="${r}" ry="${r}" fill="${theme.colors.primary}" />
    <text x="${w / 2}" y="${h / 2 + 6}" text-anchor="middle" font-family="${theme.fonts.header.family}, sans-serif" font-weight="${theme.fonts.header.weight}" font-size="36" fill="${theme.isDark ? theme.colors.text : '#FFFFFF'}">${text}</text>
  `);
}

function generateMonthTab(w: number, h: number, theme: Theme, text: string): string {
  const r = theme.borderRadius;
  return svgWrap(w, h, `
    <rect x="2" y="2" width="${w - 4}" height="${h - 4}" rx="${r}" ry="${r}" fill="${theme.colors.accent}" />
    <text x="${w / 2}" y="${h / 2 + 8}" text-anchor="middle" font-family="${theme.fonts.header.family}, sans-serif" font-weight="${theme.fonts.header.weight}" font-size="32" fill="#FFFFFF">${text}</text>
  `);
}

function generateDayTab(w: number, h: number, theme: Theme, text: string): string {
  const r = theme.borderRadius;
  return svgWrap(w, h, `
    <rect x="2" y="2" width="${w - 4}" height="${h - 4}" rx="${r}" ry="${r}" fill="${theme.colors.secondary}" />
    <text x="${w / 2}" y="${h / 2 + 6}" text-anchor="middle" font-family="${theme.fonts.body.family}, sans-serif" font-weight="${theme.fonts.body.weight}" font-size="28" fill="${theme.colors.text}">${text}</text>
  `);
}

function generateNumberCircle(w: number, h: number, theme: Theme, text: string): string {
  const cx = w / 2, cy = h / 2, r = w / 2 - 4;
  return svgWrap(w, h, `
    <circle cx="${cx}" cy="${cy}" r="${r}" fill="${theme.colors.primary}" />
    <text x="${cx}" y="${cy + 8}" text-anchor="middle" font-family="${theme.fonts.body.family}, sans-serif" font-weight="600" font-size="40" fill="${theme.isDark ? theme.colors.text : '#FFFFFF'}">${text}</text>
  `);
}

function generatePriorityMarker(w: number, h: number, theme: Theme, text: string): string {
  const colors: Record<string, string> = { P1: '#EF4444', P2: '#F59E0B', P3: '#10B981' };
  const color = colors[text] || theme.colors.accent;
  return svgWrap(w, h, `
    <rect x="2" y="2" width="${w - 4}" height="${h - 4}" rx="20" ry="20" fill="${color}" />
    <text x="${w / 2}" y="${h / 2 + 7}" text-anchor="middle" font-family="${theme.fonts.header.family}, sans-serif" font-weight="700" font-size="28" fill="#FFFFFF">${text}</text>
  `);
}

function generateCheckbox(w: number, h: number, theme: Theme, variant: number): string {
  const size = w - 16;
  const r = theme.borderRadius / 2;
  ...
  const states = [
    ...
    // 1: checked
    `<rect x="8" y="8" width="${size}" height="${size}" rx="${r}" ry="${r}" fill="${theme.colors.primary}" stroke="${stroke}" stroke-width="3" />
    ...
    // 2: partial
    `<rect x="8" y="8" width="${size}" height="${size}" rx="${r}" ry="${r}" fill="${theme.colors.secondary}" stroke="${stroke}" stroke-width="3" />
     <line x1="${w * 0.3}" y1="${h * 0.5}" x2="${w * 0.7}" y2="${h * 0.5}" stroke="${theme.colors.accent}" stroke-width="5" stroke-linecap="round" />`,
    // 3: crossed
    `<rect x="8" y="8" width="${size}" height="${size}" rx="${r}" ry="${r}" fill="none" stroke="${stroke}" stroke-width="3" />
     <line x1="${w * 0.28}" y1="${h * 0.28}" x2="${w * 0.72}" y2="${h * 0.72}" stroke="${theme.colors.accent}" stroke-width="4" stroke-linecap="round" />
     <line x1="${w * 0.72}" y1="${h * 0.28}" x2="${w * 0.28}" y2="${h * 0.72}" stroke="${theme.colors.accent}" stroke-width="4" stroke-linecap="round" />`,
  ];

  return svgWrap(w, h, states[variant % states.length]);
}

function generateProgressBar(w: number, h: number, theme: Theme, variant: number): string {
  ...
  return svgWrap(w, h, `
    <rect x="10" y="10" width="${barW}" height="${barH}" rx="${r}" ry="${r}" fill="${theme.colors.secondary}" />
    ${fillW > 0 ? `<rect x="10" y="10" width="${fillW}" height="${barH}" rx="${r}" ry="${r}" fill="${theme.colors.primary}" />` : ''}
    <text x="${w / 2}" y="${h - 4}" text-anchor="middle" font-family="${theme.fonts.body.family}, sans-serif" font-size="14" fill="${theme.colors.muted}">${pct}%</text>
  `);
}

function generateStickyNote(w: number, h: number, theme: Theme, variant: number): string {
  const colors = [theme.colors.primary, theme.colors.secondary, theme.colors.accent, theme.colors.muted];
  ...
  return svgWrap(w, h, `
    ...
    <path d="M${w - fold - 10},${h - 10} L${w - fold - 10},${h - fold - 10} L${w - 10},${h - fold - 10}" fill="none" stroke="${theme.colors.text}" stroke-opacity="0.15" stroke-width="1" />
    ${Array.from({ length: 6 }, (_, i) => {
      const y = 60 + i * 48;
      return y < h - fold - 30 ? `<line x1="35" y1="${y}" x2="${w - 35}" y2="${y}" stroke="${theme.colors.text}" stroke-opacity="0.1" stroke-width="1" />` : '';
    }).join('')}
  `);
}

function generateWashiTape(w: number, h: number, theme: Theme, variant: number): string {
  const patterns = [
    // 0: stripes
    Array.from({ length: Math.ceil(w / 20) }, (_, i) =>
      `<rect x="${i * 20}" y="0" width="10" height="${h}" fill="${theme.colors.primary}" opacity="0.5" />`
    ).join(''),
    // 1: dots
    Array.from({ length: Math.ceil(w / 30) }, (_, i) =>
      Array.from({ length: Math.ceil(h / 30) }, (_, j) =>
        `<circle cx="${i * 30 + 15}" cy="${j * 30 + 15}" r="5" fill="${theme.colors.accent}" opacity="0.4" />`
      ).join('')
    ).join(''),
    // 2: grid
    Array.from({ length: Math.ceil(w / 25) }, (_, i) =>
      `<line x1="${i * 25}" y1="0" x2="${i * 25}" y2="${h}" stroke="${theme.colors.accent}" stroke-width="0.5" opacity="0.3" />`
    ).join('') + Array.from({ length: Math.ceil(h / 25) }, (_, i) =>
      `<line x1="0" y1="${i * 25}" x2="${w}" y2="${i * 25}" stroke="${theme.colors.accent}" stroke-width="0.5" opacity="0.3" />`
    ).join(''),
    // 3: chevron
    Array.from({ length: Math.ceil(w / 40) }, (_, i) =>
      `<polyline points="${i * 40},${h} ${i * 40 + 20},0 ${i * 40 + 40},${h}" fill="none" stroke="${theme.colors.primary}" stroke-width="2" opacity="0.3" />`
    ).join(''),
...
function generateBanner(w: number, h: number, theme: Theme, text: string): string {
  const ribbonH = h - 20;
  const fold = 15;
  return svgWrap(w, h, `
    <path d="M${fold},10 L${w - fold},10 L${w},${10 + ribbonH / 2} L${w - fold},${10 + ribbonH} L${fold},${10 + ribbonH} L0,${10 + ribbonH / 2} Z" fill="${theme.colors.primary}" />
    <text x="${w / 2}" y="${h / 2 + 6}" text-anchor="middle" font-family="${theme.fonts.header.family}, sans-serif" font-weight="${theme.fonts.header.weight}" font-size="30" fill="${theme.isDark ? theme.colors.text : '#FFFFFF'}">${text}</text>
  `);
}

function generateDivider(w: number, h: number, theme: Theme, variant: number): string {
  const mid = h / 2;
  const dividers = [
    // 0: simple line
    `<line x1="20" y1="${mid}" x2="${w - 20}" y2="${mid}" stroke="${theme.colors.primary}" stroke-width="2" />`,
    // 1: double line
    `<line x1="20" y1="${mid - 3}" x2="${w - 20}" y2="${mid - 3}" stroke="${theme.colors.primary}" stroke-width="1" />
     <line x1="20" y1="${mid + 3}" x2="${w - 20}" y2="${mid + 3}" stroke="${theme.colors.primary}" stroke-width="1" />`,
    // 2: dotted
    `<line x1="20" y1="${mid}" x2="${w - 20}" y2="${mid}" stroke="${theme.colors.primary}" stroke-width="2" stroke-dasharray="6,6" />`,
  ];
  return svgWrap(w, h, dividers[variant % dividers.length]);
}

function generateFrame(w: number, h: number, theme: Theme, variant: number): string {
  const r = theme.borderRadius;
  const borders = [
    // 0: simple
    `<rect x="10" y="10" width="${w - 20}" height="${h - 20}" rx="${r}" ry="${r}" fill="none" stroke="${theme.colors.primary}" stroke-width="3" />`,
    // 1: double
    `<rect x="8" y="8" width="${w - 16}" height="${h - 16}" rx="${r}" ry="${r}" fill="none" stroke="${theme.colors.primary}" stroke-width="2" />
     <rect x="16" y="16" width="${w - 32}" height="${h - 32}" rx="${r}" ry="${r}" fill="none" stroke="${theme.colors.secondary}" stroke-width="1.5" />`,
    // 2: dashed
    `<rect x="10" y="10" width="${w - 20}" height="${h - 20}" rx="${r}" ry="${r}" fill="none" stroke="${theme.colors.primary}" stroke-width="2" stroke-dasharray="10,5" />`,
  ];
  return svgWrap(w, h, borders[variant % borders.length]);
}

function generateWeatherIcon(w: number, h: number, theme: Theme, variant: number): string {
  const cx = w / 2, cy = h / 2;
  const icons = [
    // 0: sun
    `<circle cx="${cx}" cy="${cy}" r="25" fill="${theme.colors.primary}" />
     ${Array.from({ length: 8 }, (_, i) => {
       const angle = (i * 45 * Math.PI) / 180;
       const x1 = cx + 35 * Math.cos(angle), y1 = cy + 35 * Math.sin(angle);
       const x2 = cx + 45 * Math.cos(angle), y2 = cy + 45 * Math.sin(angle);
       return `<line x1="${x1}" y1="${y1}" x2="${x2}" y2="${y2}" stroke="${theme.colors.primary}" stroke-width="4" stroke-linecap="round" />`;
     }).join('')}`,
    // 1: cloud
    `<ellipse cx="${cx - 10}" cy="${cy + 10}" rx="35" ry="25" fill="${theme.colors.secondary}" />
     <ellipse cx="${cx + 15}" cy="${cy}" rx="30" ry="28" fill="${theme.colors.primary}" />
     <ellipse cx="${cx - 25}" cy="${cy + 5}" rx="25" ry="22" fill="${theme.colors.muted}" />`,
    // 2: rain
    `<ellipse cx="${cx}" cy="${cy - 10}" rx="35" ry="22" fill="${theme.colors.secondary}" />
     ${Array.from({ length: 3 }, (_, i) =>
       `<line x1="${cx - 20 + i * 20}" y1="${cy + 20}" x2="${cx - 25 + i * 20}" y2="${cy + 40}" stroke="${theme.colors.accent}" stroke-width="3" stroke-linecap="round" />`
     ).join('')}`,
    // 3: snow
    `<ellipse cx="${cx}" cy="${cy - 10}" rx="35" ry="22" fill="${theme.colors.secondary}" />
     ${Array.from({ length: 3 }, (_, i) =>
       `<circle cx="${cx - 20 + i * 20}" cy="${cy + 30}" r="4" fill="${theme.colors.accent}" />`
     ).join('')}`,
    // 4: storm
    `<ellipse cx="${cx}" cy="${cy - 10}" rx="35" ry="22" fill="${theme.colors.muted}" />
     <polygon points="${cx - 5},${cy + 10} ${cx + 10},${cy + 10} ${cx},${cy + 35} ${cx + 15},${cy + 35} ${cx - 10},${cy + 60} ${cx - 2},${cy + 35} ${cx - 12},${cy + 35}" fill="${theme.colors.primary}" />`,
  ];
  return svgWrap(w, h, icons[variant % icons.length]);
}

function generateStarRating(w: number, h: number, theme: Theme, variant: number): string {
  const starCount = 5;
  ...
  const stars = Array.from({ length: starCount }, (_, i) => {
    ...
    const cy = h / 2;
    const fill = i < filledCount ? theme.colors.primary : theme.colors.secondary;
...
function generateArrow(w: number, h: number, theme: Theme, variant: number): string {
  const cx = w / 2, cy = h / 2;
